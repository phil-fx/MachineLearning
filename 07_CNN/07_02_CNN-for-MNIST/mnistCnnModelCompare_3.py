...
modelID = str('mnist_CNN_model_3_')
zeit = str(time.time())
...
epochs = 22
batch_size = 512 # Trade-Off Geschwind. // Performance ~2er-Potenz~ [32-1024]

# Define the DNN 
model = Sequential()

model.add(Conv2D(filters=4, kernel_size=3, padding='same', input_shape=x_train.shape[1:]))
model.add(Activation('relu'))
model.add(Conv2D(filters=5, kernel_size=3, padding='same'))
model.add(Activation('relu'))
