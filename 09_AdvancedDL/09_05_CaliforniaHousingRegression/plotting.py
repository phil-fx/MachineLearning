...
def plot_confusion_matrix(y_pred, y_true, classes_list):
    ...
    plt.imshow(cm, interpolation='nearest', cmap=plt.cm.Blues) #imshow plotte numpy arrays
    ...
    return fig  # return (nicht plot) ist wichtig - wird unten figure_fn
...
def get_occlusion(img, label, box_size, step_size, model):
    ...
    occlusion_map = np.full(shape=(rows, cols), fill_value=1.0)
    ...
    for i in range (0, rows-box_size+1, step_size):
        for j in range (0, cols-box_size+1, step_size):
            ...
            occlusion_map[i:i+step_size, j:j+step_size] = \
                np.full(shape=(step_size, step_size), fill_value=prob_right_class)
    
    fig, (ax1, ax2) = plt.subplots(nrows=1, ncols=2, figsize=(16,6))
    cMap = 'Spectral'
    ax1.imshow(img)
    heatmap = ax2.imshow(occlusion_map, cmap=cMap)
    cbar = plt.colorbar(heatmap)
    plt.show()

def get_heatmap(img, model):
    rows, cols, depth = img.shape
    heatmap_layers = [layer for layer in model.layers if 'heatmap' in layer.name]    # 1 layer hat heatmap im namen

    for layer_index, heatmap_layer in enumerate(heatmap_layers):
        heatmap_output = K.function([model.layers[0].input], [heatmap_layer.output]) # Neues Teilmodell bei dem der Output aus der 1. Layer raus kommt
        heatmap_output = heatmap_output([img.reshape(1, rows, cols, depth)])[0]      # Hier wird die predict fct des Teilmodells aufgerufen
        ...
        for filter_index, heatmap_filter in enumerate(heatmap[:num_subplots]):  # Nur die ersten 16
