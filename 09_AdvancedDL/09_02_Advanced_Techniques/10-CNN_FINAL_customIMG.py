...
modelID = str('DogsCats_CNN_Final_CustomIMG')
zeit = str(time.time())
...
images_path = os.path.abspath('/home/phil/data/Custom/')
image_names = [f for f in os.listdir(images_path) if '.jpg' in f]

for image_name in image_names:
    image_path = os.path.join(images_path, image_name)
    x = data.load_and_preprocess_custom_images(image_path)
    y_pred = model.predict(np.expand_dims(x, axis=0))[0]  # die predict-fkt gibt immer np.arrays(onehot vektor) in einer Liste aus, obwohl nur 1 Bild --> [0]
    y_pred_class = np.argmax(y_pred)
    y_pred_prob = y_pred[y_pred_class]
    plt.imshow(x)
    plt.title('predicted class: %s, probability: %f' % (data.CLASS_IDXS[y_pred_class], y_pred_prob))
    plt.show()
